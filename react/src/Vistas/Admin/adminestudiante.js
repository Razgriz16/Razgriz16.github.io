...
function Estd() {
  ...
  const handleClick = async () => {
    try {
      const response = await fetch('http://localhost:8000/generarReporte'); // Hacer la solicitud al servidor
      const data = await response.text(); // Obtener la respuesta como texto

      // Manejar la respuesta si es necesario
      console.log('Respuesta del servidor:', data);
    } catch (error) {
      console.error('Error al llamar a generarReporte:', error);
    ...
    <div className='admin-container'>
      <div className="tabla-estudiantes">
        ...
        <div className="container" style={{ marginBottom: '80px' }}>
          ...
          <Link to={`/admin/crear_estudiante`} className='btn btn-success'>
            Crear
          </Link>
