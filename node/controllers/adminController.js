...
import detalle_estudiante_ramo from "../models/detalle_estudiante_ramoModel.js"
...
export const crearEncargado = async (req, res) =>{
    try {
        await encargadoModel.create(req.body)
        res.json({
            "message" : "¡Registro creado con exito!"
        })
    } catch (error) {
...
export const crearResultados = async (req, res) =>{
    try {
        await resultadosModel.create(req.body)
        res.json({
            "message" : "¡Registro creado con exito!"
        })
    } catch (error) {
...
export const obtener1Detalle_estudiante_ramo = async (req, res) => {
    try {
        const detalle_estudiante_ramo = await detalle_estudiante_ramoModel.findAll({
            where:{ id_detalle_estudiante_ramo: req.params.id_detalle_estudiante_ramo }
        })
